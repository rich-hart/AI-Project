function [ segmented_image ] = FuzzySegmentation( image )
...
while(ttFcm<15)
    ...
    ccc1=sum(sum(u1.*u1.*image))/sum(sum(u1.*u1));
    ccc2=sum(sum(u2.*u2.*image))/sum(sum(u2.*u2));
